<?php 
    ...
    if(isset($_POST['send'])){
        ...
        {
            $GID = filter_input(INPUT_POST, "group");
            $author = GetID();
            $post_title = htmlspecialchars($_POST['title']);
            $post_message = nl2br(htmlspecialchars($_POST['message']));
            
            $store_post = $pdo->prepare("INSERT INTO posts (UID, title, message, unix, GID)VALUES(:UID, :title, :message, :time, :GID)");
            $store_post->execute([
                ":UID"     => $author,
                ":title"   => $post_title,
    ...
    $group = $pdo->prepare("SELECT GID FROM user_groups WHERE UID = :ID");
    $group->execute([
        ':ID' => $ID,
    ]);
    $membre_group = $group->fetchAll(PDO::FETCH_ASSOC);
...
<html>
    <div class="ui centered segment">
        ...
        <?php foreach ($membre_group as $value){?>
            <input type="radio" name="group" id="group" value=<?=$value["GID"]?>> Group <?=$value["GID"]?> </input>
            <?php } ?>
